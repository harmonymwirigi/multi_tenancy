...
class PaymentIntent(models.Model):
	...
	offering_type_id = models.IntegerField(null=True)
...
class ExpenditureType(models.Model):
	id = models.AutoField(primary_key=True)
	type_name = models.CharField(max_length=100)
	total_available = models.DecimalField(max_digits=15, decimal_places=2,default=0.00)
	description = models.TextField(blank=True)
	...
	@property
	def total(self):
		sum = Expenditure.objects.filter(type_id=self.id).aggregate(Sum('amount'))
		sum = sum['amount__sum'] or 0
		try:
			percentage = ((float(sum)/float(self.total_available)) * 100)
		except:
			return {"total":sum,"percentage_used":100}
		else:
			return {"total":sum,"percentage_used":percentage}
