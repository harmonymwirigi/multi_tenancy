...
class CSVLoader():
	...
	def _check_payment_method(self,file_path):
		'''
	...
	def _check_offering_type(self,file_path):
		'''
			check that the payment methods given are correct
		'''
	...
	def check_CSV(self, file_path):
		...
		if (not self._check_offering_type(file_path)):
			return False

		if (self.payment_method_column != None):
			if (not self._check_payment_method(file_path)):
				return False
