...
class Pledge(models.Model):
	...
	date = models.DateField(verbose_name='Pledge Payment Date',auto_now_add=True)
	...
	@property
	def remaining_amount(self):
		return int(self.amount) - int(self.amount_so_far)
...
class PledgePayment(models.Model):
	...
	@property
	def amount_so_far(self):
		return  PledgePayment.objects.filter(id__lte=self.id,pledge=self.pledge).aggregate(Sum('payment_amount'))['payment_amount__sum'] or 0


	@property
	def remaining_amount(self):
		return int(self.pledge.amount) - int(self.amount_so_far)

	# @property
	# def percentage_funded(self):
	# 	percent = (self.amount_so_far / self.pledge.amount) * 100
	# 	percent = floatformat(percent, 2)
	# 	return str(percent) + '%'
