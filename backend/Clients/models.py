...
class Client(TenantMixin):
    ...
    paid_until =  models.DateField(default='2024-12-12')
...
class Domain(DomainMixin):
    ...
    tenant = models.ForeignKey(
        Client,
        related_name='client_domains',  # Unique reverse accessor for Client
...
class ClientDetail(models.Model):
    ...
    def number_of_sms(self):
        with schema_context(self.client.schema_name):
            today = timezone.now()
            return SmsRecipients.objects.filter(sms__date__month=today.month,sms__date__year=today.year,status="Success").count()
...
'''
    the following is used as website content for the client church
'''
